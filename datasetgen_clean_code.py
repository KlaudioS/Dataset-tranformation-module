...
#Location of Raw CSV files Wikipedia
#path = "C:/Users/klaud/Desktop/Tese/Data/In use Datasets/Wikipedia/Configuration DS/"

#Location of Raw CSV files EDP
path = "C:/Users/klaud/Desktop/Tese/Data/In use Datasets/Wikipedia/Configuration DS/"
...
def dsConfig(maxGranularity,maxHourHistory,objectiveID):
	...
	print("fim")
	print("total de iteraçoes: " , counter)
...
def dsCreator(granularity,hourHistory,usableAttributes,norm,originalDataset,monthNum,objectiveID,name):
	...
	#print("usable att: ",usableAttributes)
	...
	if(norm == 1):
		...
		#print("tempDataset3: ",tempDataset3)
	else:
	...
	#writeCSV4(tempDataset,monthNum,norm)
	#print("DS1: ", dataset)
	#print("tempDS1: ", tempDataset) 

	
	
	##print("DS: ", dataset)
	#writeCSV4(tempDataset)
...
def addMonths(allDS,monthNumber):
	...
	#	print("1 list: ", list(allDS[actualMonth]))
	#	print("2 w/ list:", allDS[actualMonth])
		tempList = list(allDS[actualMonth])
		if(actualMonth != 0):
	#		print("enter: ",actualMonth)
	#		print("1: ",tempList[0])
			del tempList[0]
		addedMonths += tempList	
		tempList = []
	#print("DS ADDED MONTHS!")
	#print(addedMonths)
	#print("FIMMMMMMMMMMMMMM")
	return addedMonths
...
def createUsableAttList(file,objectiveID):
	...
	#print("tamanho atributos: ",attsize)
...
def deleteMetrics(metrics,tempDS):
	...
	for row in tempDS:
		row2 = list(row)
		#print("row: ",row)
		#tempRow = row2[0].split('; \,')
		#print ("temprow: ",tempRow)
		for index in sorted(indexes, reverse=True):
			#print("dataset after sorted:  ",dataset)
			#print("remove index: ",index)
	#		print("row index:", row[index])
			del row2[index]
			#print("temprow after del: ",tempRow)
		tempDataset.append(row2)
	tempDS = list(tempDataset)
	tempDataset = []
	
	#print("END DELETED TEMPDS!!!!!!!!!!!!!!!!!!!!!")
	return tempDS
...
def history(hourHistory,allDS):
	...
	for i in range(1,len(DS)):
		...
		if(tempRow != []):
			...
			tempRow=[]
		#tempRow[targetClass] + DS[hourHistory+1][targetClass]
		#tempDataset.append(tempRow)
	#print("returned Hist DS no header: ",tempDataset)	
	tempDataset.insert(0,createHistoryHeader(hourHistory,allDS))
	#print("returned Hist DS: ",tempDataset)
	return tempDataset
...
def normalization(allDS):
	...
	#print("Normalized DS: ",finalDS)
	#print("norm[0]: ",finalDS[0])
	DS= finalDS[0].tolist()
	DS.insert(0,label)
	#print("norm[0] tolist(): ",DS)
	return DS
...
def writeCSV4(tempDataset,monthNum,norm,name):
	...
	#with open(path+'attNum = '+ str(len(tempDataset[0]))+', gra= '+str(granu)+', hist= '+str(hourHis)+', addMon= '+str(monthNum) +', norm= '+str(norm) +'.csv', 'a',newline='') as f:
...
def granularityIndexer(tmpDS,granu):
	...
	tempDS = list(tmpDS)
	#print("tempDS", tempDS)
	#print("len temps ds:" ,len(tempDS))
	#print("len tempDS[0]:" ,len(tempDS[0]))
	#print("tempDS[0]:" ,tempDS[0])
	#print("linha 1", tempDS[1])
		...
		for columnIndex in range(len(tempDS[0])):
			...
			for granularityValue in range(1,granu):
				...
				if(rowIndex+granularityValue< len(tempDS)):
					...
					#print("B: ",columnIndex)
					#print("len ds[0]: ",len(tempDS[0]))
					#print("Item b4 granu increment: ",tempDS[rowIndex][columnIndex])
					#print("increment cell: ",tempDS[rowIndex+granularityValue])
					tempItem += float(tempDS[rowIndex+granularityValue][columnIndex])
					#print("tempItem: ",tempItem)
					#print("tempItem with granu: ",tempItem)
				#print("tempItem to append: ",tempItem)
				#print("ENNNNNNNNNNNNNNNNDDDDDDDDDDDDDDDDD!!!")
	...
	#print("tempDS: ", tempDS)
	#print("length tempDS: ", len(tempDS))
	return tempDS
...
def dsConfigFileFutureExactly(objectiveID,addMon,edp):
	global counter
	config =[]
	config2 = []
	allDS = []
		...
		for norm in range(2):
			...
			for att in attList:
				...
				for i in range(len(config)):
					dsCreatorFuture(int(gran[i]),int(histo[i]),att,norm,monthDS,monthNum,objectiveID,name[monthNum-1],int(futureTW[i]),edp)
...
def dsConfigFileFutureCombinations(objectiveID,addMon,edp):
	global counter
	config =[]
	config2 = []
	allDS = []
				...
				for norm in range(2):
					...
					for att in attList:
						...
						for futureTimeWindow in futureTW:
							...
							dsCreatorFuture(gra,his,att,norm,monthDS,monthNum,objectiveID,name[monthNum-1],futureTimeWindow,edp)
...
def dsCreatorFuture(granularity,hourHistory,usableAttributes,norm,originalDataset,monthNum,objectiveID,name,futureTimeWindow,edp):
	global dataset
	...
	#else:
	#	DS  = futureDS(tempDataset3,futureTimeWindow,objectiveID)
